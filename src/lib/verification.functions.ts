...
- For document_matches_recording and video_and_output_consistent: the SCREEN frames are SAMPLES taken at evenly-spaced moments across the whole recording — they are NOT the entire recording. Expect gaps. Look across ALL screen frames combined: do any of them show the document, editor, word processor, or webpage that produced the PDF? Do the visible text fragments, headings, layout, or UI chrome in the screen frames correspond to the PDF content? Pass if there is reasonable visual correspondence between what appears on screen at some point and what is in the PDF, even if individual frames are blank, show other apps, or show the editor at different stages. Only fail this check if NONE of the screen frames show any editor/document/webpage that could plausibly have produced the PDF, OR if the PDF clearly contains large bodies of text that never appear on screen in any frame. A frame showing the editor with partial content is evidence of authoring, not against it.
